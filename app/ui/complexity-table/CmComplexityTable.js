...
class CmComplexityTable extends React.Component {
    render() {
        ...
        let collections = myTable.collections.map(name => complexities.collections.find(col => col.collection == name));
        let operations = myTable.operations.map(name => complexities.operations.find(op => op.name == name));
        let cls = "complexity-table__heading";
        let headingItems = operations.map(op => React.createElement("th", {key: op.name, className: cls + "-math"}, op.name));
        ...
        let footnotes = myTable.footnotes.map(name => complexities.footnotes.find(note => note.name == name));
        let footnoteItems = 
