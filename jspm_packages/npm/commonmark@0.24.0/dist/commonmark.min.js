...
(function(process) {
    ...
    return function e(t, n, r) {
      ...
      1: [function(require, module, exports) {
        ...
        var Node = require('./node'),
            ...
            OPENTAG = require('./common').OPENTAG,
            CLOSETAG = require('./common').CLOSETAG,
            CODE_INDENT = 4,
            C_TAB = 9,
            C_NEWLINE = 10,
            C_GREATERTHAN = 62,
            ...
            reHtmlBlockClose = [/./, /<\/(?:script|pre|style)>/i, /-->/, /\?>/, />/, /\]\]>/],
 ...
 while (b);
            ...
            parseListMarker = function(parser) {
              var match,
                  nextc,
                  spacesStartCol,
                  spacesStartOffset,
                  rest = parser.currentLine.slice(parser.nextNonspace),
                  data = {
                    type: null,
                    tight: !0,
                    bulletChar: null,
              ...
              if (match = rest.match(reBulletListMarker))
                data.type = "Bullet", data.bulletChar = match[0][0];
              else {
                if (!(match = rest.match(reOrderedListMarker)))
                  return null;
                data.type = "Ordered", data.start = parseInt(match[1]), data.delimiter = match[2];
              }
              if (nextc = peek(parser.currentLine, parser.nextNonspace + match[0].length), -1 !== nextc && nextc !== C_TAB && nextc !== C_SPACE)
                return null;
              parser.advanceNextNonspace(), parser.advanceOffset(match[0].length, !0), spacesStartCol = parser.column, spacesStartOffset = parser.offset;
              do
                parser.advanceOffset(1, !0), nextc = peek(parser.currentLine, parser.offset);
 while (5 > parser.column - spacesStartCol && (nextc === C_SPACE || nextc === C_TAB));
              var blank_item = -1 === peek(parser.currentLine, parser.offset),
                  spaces_after_marker = parser.column - spacesStartCol;
              return spaces_after_marker >= 5 || 1 > spaces_after_marker || blank_item ? (data.padding = match[0].length + 1, parser.column = spacesStartCol, parser.offset = spacesStartOffset, peek(parser.currentLine, parser.offset) === C_SPACE && parser.advanceOffset(1, !0)) : data.padding = match[0].length + spaces_after_marker, data;
            },
            ...
            blocks = {
              ...
              BlockQuote: {
                "continue": function(parser) {
                  ...
                  return parser.indented || peek(ln, parser.nextNonspace) !== C_GREATERTHAN ? 1 : (parser.advanceNextNonspace(), parser.advanceOffset(1, !1), peek(ln, parser.offset) === C_SPACE && parser.offset++, 0);
                },
                finalize: function() {},
                canContain: function(t) {
                  return "Item" !== t;
              ...
              Item: {
                "continue": function(parser, container) {
                  if (parser.blank && null !== container._firstChild)
                    parser.advanceNextNonspace();
                  else {
                    if (!(parser.indent >= container._listData.markerOffset + container._listData.padding))
                      return 1;
                    parser.advanceOffset(container._listData.markerOffset + container._listData.padding, !0);
                  }
                  return 0;
                },
                finalize: function() {},
                ...
                "continue": function(parser, container) {
                  ...
                  if (container._isFenced) {
                    ...
                    for (var i = container._fenceOffset; i > 0 && peek(ln, parser.offset) === C_SPACE; )
                      parser.advanceOffset(1, !1), i--;
                  } else if (indent >= CODE_INDENT)
                    parser.advanceOffset(CODE_INDENT, !0);
                  else {
                    if (!parser.blank)
                      return 1;
                    parser.advanceNextNonspace();
              ...
              HtmlBlock: {
                "continue": function(parser, container) {
                  return !parser.blank || 6 !== container._htmlBlockType && 7 !== container._htmlBlockType ? 0 : 1;
                },
                finalize: function(parser, block) {
                  block._literal = block._string_content.replace(/(\n *)+$/, ""), block._string_content = null;
                },
            ...
            blockStarts = [function(parser) {
              return parser.indented || peek(parser.currentLine, parser.nextNonspace) !== C_GREATERTHAN ? 0 : (parser.advanceNextNonspace(), parser.advanceOffset(1, !1), peek(parser.currentLine, parser.offset) === C_SPACE && parser.advanceOffset(1, !1), parser.closeUnmatchedBlocks(), parser.addChild("BlockQuote", parser.nextNonspace), 1);
            }, function(parser) {
              var match;
              if (!parser.indented && (match = parser.currentLine.slice(parser.nextNonspace).match(reATXHeadingMarker))) {
                parser.advanceNextNonspace(), parser.advanceOffset(match[0].length, !1), parser.closeUnmatchedBlocks();
                var container = parser.addChild("Heading", parser.nextNonspace);
                return container.level = match[0].trim().length, container._string_content = parser.currentLine.slice(parser.offset).replace(/^ *#+ *$/, "").replace(/ +#+ *$/, ""), parser.advanceOffset(parser.currentLine.length - parser.offset), 2;
              }
              return 0;
            }, function(parser) {
              var match;
              if (!parser.indented && (match = parser.currentLine.slice(parser.nextNonspace).match(reCodeFence))) {
                ...
                return container._isFenced = !0, container._fenceLength = fenceLength, container._fenceChar = match[0][0], container._fenceOffset = parser.indent, parser.advanceNextNonspace(), parser.advanceOffset(fenceLength, !1), 2;
              }
              return 0;
            }, function(parser, container) {
              if (!parser.indented && peek(parser.currentLine, parser.nextNonspace) === C_LESSTHAN) {
                var blockType,
                    s = parser.currentLine.slice(parser.nextNonspace);
                for (blockType = 1; 7 >= blockType; blockType++)
                  if (reHtmlBlockOpen[blockType].test(s) && (7 > blockType || "Paragraph" !== container.type)) {
                    parser.closeUnmatchedBlocks();
                    var b = parser.addChild("HtmlBlock", parser.offset);
                    return b._htmlBlockType = blockType, 2;
                  }
              }
              return 0;
            }, function(parser, container) {
              var match;
              if (!parser.indented && "Paragraph" === container.type && (match = parser.currentLine.slice(parser.nextNonspace).match(reSetextHeadingLine))) {
                parser.closeUnmatchedBlocks();
                var heading = new Node("Heading", container.sourcepos);
                return heading.level = "=" === match[0][0] ? 1 : 2, heading._string_content = container._string_content, container.insertAfter(heading), container.unlink(), parser.tip = heading, parser.advanceOffset(parser.currentLine.length - parser.offset, !1), 2;
              }
              return 0;
            }, function(parser) {
              return !parser.indented && reThematicBreak.test(parser.currentLine.slice(parser.nextNonspace)) ? (parser.closeUnmatchedBlocks(), parser.addChild("ThematicBreak", parser.nextNonspace), parser.advanceOffset(parser.currentLine.length - parser.offset, !1), 2) : 0;
            }, function(parser, container) {
              var data;
              return parser.indented && "List" !== container.type || !(data = parseListMarker(parser)) ? 0 : (parser.closeUnmatchedBlocks(), "List" === parser.tip.type && listsMatch(container._listData, data) || (container = parser.addChild("List", parser.nextNonspace), container._listData = data), container = parser.addChild("Item", parser.nextNonspace), container._listData = data, 1);
            }, function(parser) {
              return parser.indented && "Paragraph" !== parser.tip.type && !parser.blank ? (parser.advanceOffset(CODE_INDENT, !0), parser.closeUnmatchedBlocks(), parser.addChild("CodeBlock", parser.offset), 2) : 0;
            }],
            advanceOffset = function(count, columns) {
              for (var charsToTab,
                  c,
                  cols = 0,
                  currentLine = this.currentLine; count > 0 && (c = currentLine[this.offset]); )
                "	" === c ? (charsToTab = 4 - this.column % 4, this.column += charsToTab, this.offset += 1, count -= columns ? charsToTab : 1) : (cols += 1, this.offset += 1, this.column += 1, count -= 1);
            },
            advanceNextNonspace = function() {
            ...
            findNextNonspace = function() {
              for (var c,
                  ...
                  i = this.offset,
                  cols = this.column; "" !== (c = currentLine.charAt(i)); )
                if (" " === c)
                  i++, cols++;
                else {
                  if ("	" !== c)
              ...
              this.blank = "\n" === c || "\r" === c || "" === c, this.nextNonspace = i, this.nextNonspaceColumn = cols, this.indent = this.nextNonspaceColumn - this.column, this.indented = this.indent >= CODE_INDENT;
            },
            incorporateLine = function(ln) {
              var t,
                  all_matched = !0,
                  container = this.doc;
              this.oldtip = this.tip, this.offset = 0, this.column = 0, this.lineNumber += 1, -1 !== ln.indexOf("\x00") && (ln = ln.replace(/\0/g, "�")), this.currentLine = ln;
              ...
              this.allClosed = container === this.oldtip, this.lastMatchedContainer = container, this.blank && container._lastLineBlank && (this.breakOutOfLists(container), container = this.tip);
              for (var matchedLeaf = "Paragraph" !== container.type && blocks[container.type].acceptsLines,
                ...
                if (this.findNextNonspace(), !this.indented && !reMaybeSpecial.test(ln.slice(this.nextNonspace))) {
              ...
              if (this.allClosed || this.blank || "Paragraph" !== this.tip.type) {
                ...
                this.blocks[t].acceptsLines ? (this.addLine(), "HtmlBlock" === t && container._htmlBlockType >= 1 && 5 >= container._htmlBlockType && reHtmlBlockClose[container._htmlBlockType].test(this.currentLine.slice(this.offset)) && this.finalize(container, this.lineNumber)) : ln.length > this.offset && !this.blank && (container = this.addChild("Paragraph", this.offset), this.advanceNextNonspace(), this.addLine());
              } else
                this.addLine();
            ...
            processInlines = function(block) {
              var node,
                  ...
                  t,
                  walker = block.walker();
              for (this.inlineParser.refmap = this.refmap, this.inlineParser.options = this.options; event = walker.next(); )
                node = event.node, t = node.type, event.entering || "Paragraph" !== t && "Heading" !== t || this.inlineParser.parse(node);
            },
            Document = function() {
            ...
            parse = function(input) {
              this.doc = new Document, this.tip = this.doc, this.refmap = {}, this.lineNumber = 0, this.lastLineLength = 0, this.offset = 0, this.column = 0, this.lastMatchedContainer = this.doc, this.currentLine = "", this.options.time && console.time("preparing input");
      ...
      2: [function(require, module, exports) {
        ...
        var encode = require('mdurl/encode'),
            ...
            C_BACKSLASH = 92,
            decodeHTML = require('entities').decodeHTML,
            ...
            unescapeChar = function(s) {
              return s.charCodeAt(0) === C_BACKSLASH ? s.charAt(1) : decodeHTML(s);
            },
              ...
              for (options.time && console.time("rendering"); event = walker.next(); ) {
                ...
                switch (node.type) {
                  ...
                  case "Strong":
                    ...
                    break;
                  case "HtmlInline":
                    out(options.safe ? "<!-- raw HTML omitted -->" : node.literal);
                    break;
                  case "CustomInline":
                    entering && node.onEnter ? out(node.onEnter) : !entering && node.onExit && out(node.onExit);
                    break;
                  case "Link":
                    entering ? (options.safe && potentiallyUnsafe(node.destination) || attrs.push(["href", esc(node.destination, !0)]), node.title && attrs.push(["title", esc(node.title, !0)]), out(tag("a", attrs))) : out(tag("/a"));
                    break;
                  case "Image":
                    entering ? (0 === disableTags && out(options.safe && potentiallyUnsafe(node.destination) ? '<img src="" alt="' : '<img src="' + esc(node.destination, !0) + '" alt="'), disableTags += 1) : (disableTags -= 1, 0 === disableTags && (node.title && out('" title="' + esc(node.title, !0)), out('" />')));
                    break;
                  case "Code":
                  ...
                  case "Heading":
                    ...
                    break;
                  case "CodeBlock":
                    info_words = node.info ? node.info.split(/\s+/) : [], info_words.length > 0 && info_words[0].length > 0 && attrs.push(["class", "language-" + esc(info_words[0], !0)]), cr(), out(tag("pre") + tag("code", attrs)), out(esc(node.literal, !1)), out(tag("/code") + tag("/pre")), cr();
                    break;
                  case "HtmlBlock":
                    cr(), out(options.safe ? "<!-- raw HTML omitted -->" : node.literal), cr();
                    break;
                  case "CustomBlock":
                    cr(), entering && node.onEnter ? out(node.onEnter) : !entering && node.onExit && out(node.onExit), cr();
                    break;
                  case "ThematicBreak":
      ...
      6: [function(require, module, exports) {
        ...
        var C_NEWLINE = 10,
            ...
            C_SINGLEQUOTE = 39,
            C_DOUBLEQUOTE = 34,
            ESCAPABLE = common.ESCAPABLE,
            ESCAPED_CHAR = "\\\\" + ESCAPABLE,
            REG_CHAR = "[^\\\\()\\x00-\\x20]",
            IN_PARENS_NOSP = "\\((" + REG_CHAR + "|" + ESCAPED_CHAR + "|\\\\)*\\)",
            ENTITY = common.ENTITY,
            reHtmlTag = common.reHtmlTag,
            ...
            reAutolink = /^<[A-Za-z][A-Za-z0-9.+-]{1,31}:[^<>\x00-\x20]*>/i,
            reSpnl = /^ *(?:\n *)?/,
            reWhitespaceChar = /^\s/,
            reWhitespace = /\s+/g,
            reFinalSpace = / *$/,
            reInitialSpace = /^ */,
            reSpaceAtEndOfLine = /^ *(?:\n|$)/,
            reLinkLabel = new RegExp("^\\[(?:[^\\\\\\[\\]]|" + ESCAPED_CHAR + "|\\\\){0,1000}\\]"),
            reMain = /^[^\n`\[\]\\!<&*_'"]+/m,
            text = function(s) {
            ...
            removeDelimitersBetween = function(bottom, top) {
              bottom.next !== top && (bottom.next = top, top.previous = bottom);
            },
            processEmphasis = function(stack_bottom) {
              var opener,
                  ...
                  opener_found,
                  openers_bottom = [];
              for (openers_bottom[C_UNDERSCORE] = stack_bottom, openers_bottom[C_ASTERISK] = stack_bottom, openers_bottom[C_SINGLEQUOTE] = stack_bottom, openers_bottom[C_DOUBLEQUOTE] = stack_bottom, closer = this.delimiters; null !== closer && closer.previous !== stack_bottom; )
              ...
              for (; null !== closer; ) {
                ...
                if (!closer.can_close || closercc !== C_UNDERSCORE && closercc !== C_ASTERISK && closercc !== C_SINGLEQUOTE && closercc !== C_DOUBLEQUOTE)
                  ...
                  for (opener = closer.previous, opener_found = !1; null !== opener && opener !== stack_bottom && opener !== openers_bottom[closercc]; ) {
                  ...
                  if (old_closer = closer, closercc === C_ASTERISK || closercc === C_UNDERSCORE)
                    if (opener_found) {
                      ...
                      opener_inl.insertAfter(emph), removeDelimitersBetween(opener, closer), 0 === opener.numdelims && (opener_inl.unlink(), this.removeDelimiter(opener)), 0 === closer.numdelims && (closer_inl.unlink(), tempstack = closer.next, this.removeDelimiter(closer), closer = tempstack);
                    } else
                      closer = closer.next;
                  else
                    closercc === C_SINGLEQUOTE ? (closer.node._literal = "’", opener_found && (opener.node._literal = "‘"), closer = closer.next) : closercc === C_DOUBLEQUOTE && (closer.node._literal = "”", opener_found && (opener.node.literal = "“"), closer = closer.next);
                  opener_found || (openers_bottom[closercc] = old_closer.previous, old_closer.can_open || this.removeDelimiter(old_closer));
                }
              }
              for (; null !== this.delimiters && this.delimiters !== stack_bottom; )
            ...
            parseLinkLabel = function() {
              ...
              return null === m || m.length > 1001 ? 0 : m.length;
            },
            ...
            parseCloseBracket = function(block) {
              ...
              if (is_image = opener.cc === C_BANG, this.peek() === C_OPEN_PAREN)
                this.pos++, this.spnl() && null !== (dest = this.parseLinkDestination()) && this.spnl() && (reWhitespaceChar.test(this.subject.charAt(this.pos - 1)) && (title = this.parseLinkTitle()), !0) && this.spnl() && this.peek() === C_CLOSE_PAREN && (this.pos += 1, matched = !0);
              else {
              ...
              if (matched) {
                ...
                if (block.appendChild(node), this.processEmphasis(opener.previous), opener.node.unlink(), !is_image)
            ...
            parseEntity = function(block) {
              var m;
              return (m = this.match(reEntityHere)) ? (block.appendChild(text(decodeHTML(m))), !0) : !1;
            },
            parseString = function(block) {
              ...
              return (m = this.match(reMain)) ? (block.appendChild(text(this.options.smart ? m.replace(reEllipses, "…").replace(reDash, function(chars) {
                var enCount = 0,
                    emCount = 0;
                return chars.length % 3 === 0 ? emCount = chars.length / 3 : chars.length % 2 === 0 ? enCount = chars.length / 2 : chars.length % 3 === 2 ? (enCount = 1, emCount = (chars.length - 2) / 3) : (enCount = 2, emCount = (chars.length - 4) / 3), "—".repeat(emCount) + "–".repeat(enCount);
              }) : m)), !0) : !1;
            },
            parseNewline = function(block) {
              this.pos += 1;
              var lastc = block._lastChild;
              if (lastc && "Text" === lastc.type && " " === lastc._literal[lastc._literal.length - 1]) {
                var hardbreak = " " === lastc._literal[lastc._literal.length - 2];
                lastc._literal = lastc._literal.replace(reFinalSpace, ""), block.appendChild(new Node(hardbreak ? "Hardbreak" : "Softbreak"));
              } else
            ...
            parseReference = function(s, refmap) {
              ...
              this.spnl(), title = this.parseLinkTitle(), null === title && (title = "", this.pos = beforetitle);
              var atLineEnd = !0;
              if (null === this.match(reSpaceAtEndOfLine) && ("" === title ? atLineEnd = !1 : (title = "", this.pos = beforetitle, atLineEnd = null !== this.match(reSpaceAtEndOfLine))), !atLineEnd)
                return this.pos = startpos, 0;
              var normlabel = normalizeReference(rawlabel);
              return "" === normlabel ? (this.pos = startpos, 0) : (refmap[normlabel] || (refmap[normlabel] = {
                destination: dest,
                title: title
              }), this.pos - startpos);
            },
            ...
            parseInlines = function(block) {
              for (this.subject = block._string_content.trim(), this.pos = 0, this.delimiters = null; this.parseInline(block); )
                ;
              block._string_content = null, this.processEmphasis(null);
            };
        ...
        var resumeAt = function(node, entering) {
          this.current = node, this.entering = entering === !0;
            ...
            next = function() {
              ...
              return entering && container ? cur._firstChild ? (this.current = cur._firstChild, this.entering = !0) : this.entering = !1 : cur === this.root ? this.current = null : null === cur._next ? (this.current = cur._parent, this.entering = !1) : (this.current = cur._next, this.entering = !0), {
                entering: entering,
                node: cur
              };
            },
        ...
        var escapeXml = require('./common').escapeXml,
            ...
            renderNodes = function(block) {
              ...
              for (options.time && console.time("rendering"), buffer += '<?xml version="1.0" encoding="UTF-8"?>\n', buffer += '<!DOCTYPE CommonMark SYSTEM "CommonMark.dtd">\n'; event = walker.next(); )
                if (entering = event.entering, node = event.node, nodetype = node.type, container = node.isContainer, selfClosing = "ThematicBreak" === nodetype || "Hardbreak" === nodetype || "Softbreak" === nodetype, tagname = toTagName(nodetype), entering) {
                  switch (attrs = [], nodetype) {
                    ...
                    case "CustomInline":
                    case "CustomBlock":
                      attrs.push(["on_enter", node.onEnter]), attrs.push(["on_exit", node.onExit]);
                  }
                  ...
                  if (cr(), out(tag(tagname, attrs, selfClosing)), container)
                    ...
                    lit && out(esc(lit)), out(tag("/" + tagname));
                  }
                } else
      ...
      10: [function(require, module, exports) {
        ...
        exports.decode = function(data, level) {
          return (!level || 0 >= level ? decode.XML : decode.HTML)(data);
        }, exports.decodeStrict = function(data, level) {
          return (!level || 0 >= level ? decode.XML : decode.HTMLStrict)(data);
        }, exports.encode = function(data, level) {
          return (!level || 0 >= level ? encode.XML : encode.HTML)(data);
      ...
      11: [function(require, module, exports) {
        ...
        function getReplacer(map) {
          return function(str) {
            return "#" === str.charAt(1) ? decodeCodePoint("X" === str.charAt(2) || "x" === str.charAt(2) ? parseInt(str.substr(3), 16) : parseInt(str.substr(2), 10)) : map[str.slice(1, -1)];
          };
        }
        var entityMap = require('../maps/entities.json!systemjs-json'),
            ...
            decodeHTML = function() {
              ...
              for (var legacy = Object.keys(legacyMap).sort(sorter),
      ...
      12: [function(require, module, exports) {
        function decodeCodePoint(codePoint) {
          if (codePoint >= 55296 && 57343 >= codePoint || codePoint > 1114111)
            return "�";
      ...
      13: [function(require, module, exports) {
        ...
        function astralReplacer(c) {
          var high = c.charCodeAt(0),
              low = c.charCodeAt(1),
              codePoint = 1024 * (high - 55296) + low - 56320 + 65536;
        ...
        var re_nonASCII = /[^\0-\x7F]/g,
            re_astralSymbols = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g,
            re_xmlChars = getInverseReplacer(inverseXML);
      ...
      18: [function(require, module, exports) {
        ...
        function getDecodeCache(exclude) {
          ...
          if (cache)
            return cache;
          for (cache = decodeCache[exclude] = [], i = 0; 128 > i; i++)
            ch = String.fromCharCode(i), cache.push(ch);
          for (i = 0; exclude.length > i; i++)
            ch = exclude.charCodeAt(i), cache[ch] = "%" + ("0" + ch.toString(16).toUpperCase()).slice(-2);
          return cache;
        }
        function decode(string, exclude) {
          var cache;
          return "string" != typeof exclude && (exclude = decode.defaultChars), cache = getDecodeCache(exclude), string.replace(/(%[a-f0-9]{2})+/gi, function(seq) {
            var i,
                l,
                b1,
                b2,
      ...
      19: [function(require, module, exports) {
        ...
        function getEncodeCache(exclude) {
          ...
          if (cache)
            return cache;
          for (cache = encodeCache[exclude] = [], i = 0; 128 > i; i++)
            ch = String.fromCharCode(i), cache.push(/^[0-9a-z]$/i.test(ch) ? ch : "%" + ("0" + i.toString(16).toUpperCase()).slice(-2));
          for (i = 0; exclude.length > i; i++)
        ...
        function encode(string, exclude, keepEscaped) {
          var i,
              ...
              cache,
              result = "";
          for ("string" != typeof exclude && (keepEscaped = exclude, exclude = encode.defaultChars), "undefined" == typeof keepEscaped && (keepEscaped = !0), cache = getEncodeCache(exclude), i = 0, l = string.length; l > i; i++)
            if (code = string.charCodeAt(i), keepEscaped && 37 === code && l > i + 2 && /^[0-9a-f]{2}$/i.test(string.slice(i + 1, i + 3)))
              result += string.slice(i, i + 3), i += 2;
            else if (128 > code)
              result += cache[code];
            else if (code >= 55296 && 57343 >= code) {
              if (code >= 55296 && 56319 >= code && l > i + 1 && (nextCode = string.charCodeAt(i + 1), nextCode >= 56320 && 57343 >= nextCode)) {
        ...
        var encodeCache = {};
        encode.defaultChars = ";/?:@&=+$,-_.!~*'()#", encode.componentChars = "-_.!~*'()", module.exports = encode;
      }, {}],
          ...
          var defineProperty = function() {
            ...
            return result;
              ...
              repeat = function(count) {
                ...
                if (n != n && (n = 0), 0 > n || n == 1 / 0)
                  throw RangeError();
                for (var result = ""; n; )
                  n % 2 == 1 && (result += string), n > 1 && (string += string), n >>= 1;
                return result;
              };
          defineProperty ? defineProperty(String.prototype, "repeat", {
            value: repeat,
            configurable: !0,
            writable: !0
