  ...
  (function(AJAX, HUB, HTML, SVG) {
    ...
    SVG.Augment({
      ...
      Startup: function() {
        ...
        this.hiddenDiv = HTML.Element("div", {style: {
            visibility: "hidden",
            overflow: "hidden",
            position: "absolute",
            top: 0,
        ...
        BBOX.GLYPH.defs = this.addElement(this.addElement(this.hiddenDiv.parentNode, "svg"), "defs", {id: "MathJax_SVG_glyphs"});
        this.ExSpan = HTML.Element("span", {style: {
            position: "absolute",
            "font-size-adjust": "none"
          }}, [["span", {className: "MathJax_SVG_ExBox"}]]);
        this.linebreakSpan = HTML.Element("span", null, [["hr", {style: {
            width: "auto",
            size: 1,
            padding: 0,
            border: 0,
      ...
      preTranslate: function(state) {
        ...
        for (i = 0; i < m; i++) {
          ...
          jax.SVG.ex = ex;
          jax.SVG.em = em = ex / SVG.TeX.x_height * 1000;
      ...
      postTranslate: function(state, partial) {
        ...
        if (!this.hideProcessedMath)
          return;
        for (var i = state.SVGlast,
            m = state.SVGeqn; i < m; i++) {
    ...
    BBOX.G = BBOX;
    BBOX.NULL = BBOX.Subclass({Init: function() {
    ...
    HUB.Register.StartupHook("mml Jax Ready", function() {
      ...
      MML.chars.Augment({toSVG: function(variant, scale, remap, chars) {
      ...
      MML.entity.Augment({toSVG: function(variant, scale, remap, chars) {
      ...
      MML.mo.Augment({
        toSVG: function(HW, D) {
          ...
          if (this.data.join("").length === 1 && parent && parent.isa(MML.munderover) && this.CoreText(parent.data[parent.base]).length === 1) {
      ...
      MML.mtext.Augment({toSVG: function() {
      ...
      MML.mspace.Augment({toSVG: function() {
          this.SVGgetStyles();
      ...
      MML.mphantom.Augment({toSVG: function(HW, D) {
          this.SVGgetStyles();
          var svg = this.SVG();
          this.SVGgetScale(svg);
          if (this.data[0] != null) {
      ...
      MML.mfrac.Augment({
        toSVG: function() {
          ...
          if (values.bevelled) {
            ...
            v *= scale;
            if (t === 0) {
              p = Math.max((isDisplay ? 7 : 3) * SVG.TeX.rule_thickness, 2 * mt);
              q = (u - num.d) - (den.h - v);
              if (q < p) {
                u += (p - q) / 2;
                v += (p - q) / 2;
      ...
      MML.munderover.Augment({toSVG: function(HW, D) {
          this.SVGgetStyles();
      ...
      MML.msubsup.Augment({toSVG: function(HW, D) {
          this.SVGgetStyles();
          var svg = this.SVG(),
              scale = this.SVGgetScale(svg);
          this.SVGhandleSpace(svg);
      ...
      MML.math.Augment({
        ...
        toSVG: function(span, div) {
          ...
          if (this.data[0]) {
            ...
            var H = (Math.ceil(svg.H / px) + 1) * px + SVG.HFUZZ,
            ...
            svg.element.setAttribute("viewBox", SVG.Fixed(-l, 1) + " " + SVG.Fixed(-H, 1) + " " + SVG.Fixed(l + svg.w + r, 1) + " " + SVG.Fixed(H + D, 1));
            if (svg.H > svg.h)
              style.marginTop = SVG.Ex(svg.h - H);
            if (svg.D > svg.d) {
              ...
              if (shift) {
                HUB.Insert(style, ({
                  ...
                  right: {
                    ...
                    marginLeft: SVG.Ex(Math.max(0, shift - (l + svg.w + r)))
                  },
                  center: {
      ...
      MML.TeXAtom.Augment({toSVG: function(HW, D) {
          this.SVGgetStyles();
          var svg = this.SVG();
          this.SVGhandleSpace(svg);
          if (this.data[0] != null) {
    ...
    HUB.Browser.Select({Opera: function(browser) {
        SVG.Augment({operaZoomRefresh: true});
      }});
    ...
    if (!document.createElementNS) {
      ...
      SVG.Augment({Element: function(type, def) {
