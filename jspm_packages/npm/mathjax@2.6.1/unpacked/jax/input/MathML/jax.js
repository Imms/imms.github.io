  ...
  (function(MATHML, BROWSER) {
    ...
    MATHML.Parse = MathJax.Object.Subclass({
      ...
      Parse: function(math, script) {
        ...
        if (err)
          MATHML.Error(["ParsingError", "Error parsing MathML: %1", err.textContent.replace(/This page.*?errors:|XML Parsing Error: |Below is a rendering of the page.*/g, "")]);
        ...
        if (doc.firstChild.nodeName.toLowerCase() === "html") {
          ...
          if (h1 && h1.textContent === "XML parsing error" && h1.nextSibling)
            MATHML.Error(["ParsingError", "Error parsing MathML: %1", String(h1.nextSibling.nodeValue).replace(/fatal parsing error: /, "")]);
        }
        if (doc.firstChild.nodeName.toLowerCase().replace(/^[a-z]+:/, "") !== "math") {
          MATHML.Error(["MathMLRootElement", "MathML must be formed by a <math> element, not %1", "<" + doc.firstChild.nodeName + ">"]);
        }
        var data = {
          math: doc.firstChild,
          script: script
      ...
      MakeMML: function(node) {
        var CLASS = String(node.getAttribute("class") || "");
        var mml,
      ...
      AddAttributes: function(mml, node) {
        ...
        mml.attrNames = [];
        for (var i = 0,
            m = node.attributes.length; i < m; i++) {
    ...
    MATHML.Augment({
      ...
      createMSParser: function() {
        var parser = null;
        var xml = ["MSXML2.DOMDocument.6.0", "MSXML2.DOMDocument.5.0", "MSXML2.DOMDocument.4.0", "MSXML2.DOMDocument.3.0", "MSXML2.DOMDocument.2.0", "Microsoft.XMLDOM"];
        for (var i = 0,
            m = xml.length; i < m && !parser; i++) {
          try {
      ...
      parserCreationError: function() {
        alert(_("CantCreateXMLParser", "MathJax can't create an XML parser for MathML.  Check that\n" + "the 'Script ActiveX controls marked safe for scripting' security\n" + "setting is enabled (use the Internet Options item in the Tools\n" + "menu, and select the Security panel, then press the Custom Level\n" + "button to check this).\n\n" + "MathML equations will not be able to be processed by MathJax."));
      },
      Startup: function() {
    ...
    MATHML.prefilterHooks.Add(function(data) {
      data.math = (typeof(data.math) === "string" ? MATHML.prefilterMath(data.math, data.script) : MATHML.prefilterMathML(data.math, data.script));
    });
    MATHML.Parse.Entity = {
      ApplyFunction: '\u2061',
      Backslash: '\u2216',
