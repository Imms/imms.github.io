...
(function(process) {
  (function(TEX, HUB, AJAX) {
    ...
    var TEXDEF = {Add: function(src, dst, nouser) {
        if (!dst) {
          dst = this;
        }
        for (var id in src) {
    ...
    var PARSE = MathJax.Object.Subclass({
      ...
      csDelimiter: function(name, delim) {
        ...
        this.Push(this.mmlToken(MML.mo(delim).With({
          fence: false,
          stretchy: false
        }).With(def)));
      },
      ...
      Superscript: function(c) {
        ...
        var movesupsub = base.movesupsub,
            position = base.sup;
        if ((base.type === "msubsup" && base.data[base.sup]) || (base.type === "munderover" && base.data[base.over] && !base.subsupOK)) {
      ...
      Subscript: function(c) {
        ...
        var movesupsub = base.movesupsub,
            position = base.sub;
        if ((base.type === "msubsup" && base.data[base.sub]) || (base.type === "munderover" && base.data[base.under] && !base.subsupOK)) {
      ...
      mi2mo: function(mi) {
        ...
        mo.useMMLspacing &= ~(mo.SPACE_ATTR.lspace | mo.SPACE_ATTR.rspace);
        return mo;
      },
      Comment: function(c) {
      ...
      SetStyle: function(name, texStyle, style, level) {
        ...
        this.Push(STACKITEM.style().With({styles: {
            displaystyle: style,
            scriptlevel: level
          }}));
      },
      SetSize: function(name, size) {
        ...
        this.Push(STACKITEM.style().With({styles: {mathsize: size + "em"}}));
      },
      Color: function(name) {
      ...
      UnderOver: function(name, c, stack, noaccent) {
        ...
        mml.SetData(mml[pos], this.mmlToken(MML.mo(MML.entity("#x" + c)).With({
          stretchy: true,
          accent: !noaccent
        })));
        if (stack) {
      ...
      Array: function(begin, open, close, align, spacing, vspacing, style, raggedHeight) {
        ...
        var array = STACKITEM.array().With({arraydef: {
            columnalign: align,
            columnspacing: (spacing || "1em"),
            rowspacing: (vspacing || "4pt")
          }});
    ...
    TEX.Augment({
      ...
      combineRelations: function(mml) {
        ...
        for (i = 0, m = mml.data.length; i < m; i++) {
          if (mml.data[i]) {
            if (mml.isa(MML.mrow)) {
              while (i + 1 < m && (m1 = mml.data[i]) && (m2 = mml.data[i + 1]) && m1.isa(MML.mo) && m2.isa(MML.mo) && m1.Get("texClass") === MML.TEXCLASS.REL && m2.Get("texClass") === MML.TEXCLASS.REL) {
                if (m1.variantForm == m2.variantForm && m1.Get("mathvariant") == m2.Get("mathvariant") && m1.style == m2.style && m1["class"] == m2["class"] && !m1.id && !m2.id) {
